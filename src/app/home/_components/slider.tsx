...
export default function SliderList({ services }: ServicesData) {
  ...
  const next = () => {
    if (sliderEl) sliderEl.slickNext();
    currentIndex < services.length - 1
      ? setCurrentIndex((currentIndex) => currentIndex + 1)
      : setCurrentIndex(0);
  };
  const previous = () => {
    if (sliderEl) sliderEl.slickPrev();
    currentIndex > 0
      ? setCurrentIndex((currentIndex) => currentIndex - 1)
    ...
    <section className="overflow-hidden">
      ...
      <div className="flex gap-8 items-center justify-between">
        ...
        <div className="flex gap-4 overflow-visible">
          <button onClick={previous} className={`shrink-0`}>
            <Image
              src="./icons/right.svg"
              alt="prev"
              width={44}
          ...
          <button onClick={next} className={`shrink-0`}>
            <Image src="./icons/right.svg" alt="prev" width={44} height={44} />
          </button>
        </div>
      </div>
    </section>
